...
- **Dark theme only**; `prefers-reduced-motion` handled globally in `src/index.css` (kills all transition/animation durations *and* delays); focus-visible outlines for keyboard users.
- **Animations**: scroll-reveal via the `useReveal` hook (`src/hooks/useReveal.js`) — returns `[ref, revealCls]`, pairs with `.reveal`/`.reveal--visible` in `index.css`; fires once per element (IntersectionObserver, threshold 0.1). Load-entrance via `animate-rise` utility (keyframes in `@theme`) staggered with `[animation-delay:...]` — used on the Home hero and About page top section. The Home hero title uses `Typewriter` (`src/components/Typewriter.jsx`): per-character typing with a blinking `animate-caret` cursor, an invisible full-text copy to prevent layout shift, `sr-only` full text for screen readers, and instant text under `prefers-reduced-motion`. Hover micro-interactions live in `index.css` (`.skill-pill` lift, `.link-underline span` arrow shift) and `ProjectCard` (image zoom via `group-hover:scale-105` inside an `overflow-hidden` wrapper).
